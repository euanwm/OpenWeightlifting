...
class QueryThis:
    ...
    def generate_gender_cats(self):
        """Yep"""
        with open("/Users/euanmeston/PycharmProjects/OpenWeightlifting/database_handler/gender_categories.json",
    ...
    def sort_by_total(self, gender='female'):
        filpe = os.path.join(self.query_folder, gender + ".csv")
        db_list = load_csv_as_list(filpe)
        shite = sorted(db_list, key=lambda x: int(x[13]), reverse=True)
        for x in shite:
            for count, y in enumerate(shite):
                if x[3] == y[3] and x[14] == y[14] and x != y:
                    shite.pop(count)
        for x in shite[:110:]:
            append_to_csv(os.path.join(self.query_folder, f"top_100_{gender}.csv"), x)

if __name__ == '__main__':
    query_folder_root = "/Users/euanmeston/PycharmProjects/OpenWeightlifting/queries"
    res_root = "/Users/euanmeston/PycharmProjects/OpenWeightlifting/database_root"
